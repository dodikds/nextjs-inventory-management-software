...
// Reused as-is from Purchases — the modal is generic (product name + a
// unit-cost/tax/discount/unit draft), nothing about it is Purchase-specific,
// so a duplicate PurchaseReturnItemModal would just be the same 166 lines
// twice.
import PurchaseItemModal, { type PurchaseItemModalValues } from "@/components/purchases/PurchaseItemModal";
import styles from "./PurchaseReturnForm.module.css";

type OptionItem = { id: string; name: string };

type PurchaseReturnStatus = "RECEIVED" | "PENDING" | "ORDERED";

// Every new line starts at zero discount/tax, matching design/Create
// Purchase Return.html's own example row — a line only picks up a
// discount/tax once the pencil-icon modal is used to set one.
type PurchaseReturnItemState = {
  productId: string;
  code: string;
  name: string;
  unitCost: string;
  /** Current quantity in the selected warehouse — the ceiling this line can return, enforced server-side on save. */
  stock: number;
  quantity: number;
...
// app/(dashboard)/purchases/returns/[id]/edit/page.tsx (a later step), which
// will build this from the stored PurchaseReturn and its items.
export type PurchaseReturnFormInitialData = {
  id: string;
  date: string;
  warehouseId: string;
  supplierId: string;
  items: PurchaseReturnItemState[];
...
type PurchaseReturnFormProps = {
  ...
  units: OptionItem[];
  initialData?: PurchaseReturnFormInitialData;
};
...
function lineTotals(item: PurchaseReturnItemState) {
...
export default function PurchaseReturnForm({ warehouses, suppliers, units, initialData }: PurchaseReturnFormProps) {
  ...
  const [items, setItems] = useState<PurchaseReturnItemState[]>(initialData?.items ?? []);
  ...
  const [status, setStatus] = useState<PurchaseReturnStatus>(initialData?.status ?? "RECEIVED");
  const [notes, setNotes] = useState(initialData?.notes ?? "");

  const [editingProductId, setEditingProductId] = useState<string | null>(null);

  const [searchQuery, setSearchQuery] = useState("");
  const [searchResults, setSearchResults] = useState<PurchaseReturnProductSearchResult[]>([]);
  ...
  function runSearch(query: string, forWarehouseId: string) {
    ...
    startSearchTransition(async () => {
      const results = await searchProductsForPurchaseReturn(query.trim(), forWarehouseId);
      setSearchResults(results);
    });
  }

  function handleWarehouseChange(next: string) {
    // Unlike Purchases (where a product qualifies regardless of warehouse),
    // this form's product search — and every added line's "Stock" ceiling —
    // is scoped to one warehouse: you can only return what that warehouse
    // holds. Switching warehouses mid-return would leave stale stock
    // figures (and possibly lines the new warehouse can't even cover)
    // around, so the slate is wiped instead — same as Adjustments'
  ...
  function handleSelectProduct(product: PurchaseReturnProductSearchResult) {
    if (items.some((item) => item.productId === product.id)) {
      toast.error(`${product.name} is already in this return`);
      return;
    }
    setItems((prev) => [
      ...prev,
  ...
  function handleCancel() {
    ...
    router.push(isEditing ? `/purchases/returns/${initialData.id}` : "/purchases/returns");
  }
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="gg-field" style={{ marginTop: "var(--sp-6)", maxWidth: 420 }}>
          ...
          <select
            ...
            className="gg-select"
            value={status}
            onChange={(e) => setStatus(e.target.value as PurchaseReturnStatus)}
          >
