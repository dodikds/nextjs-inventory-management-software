...
type SaleReturnsErrorProps = {
  error: Error & { digest?: string };
  reset: () => void;
};

export default function SaleReturnsError({ error, reset }: SaleReturnsErrorProps) {
  useEffect(() => {
    console.error(error);
  }, [error]);
  ...
  return (
    <div className="gg-card gg-card-pad" style={{ textAlign: "center", padding: "var(--sp-12) var(--sp-6)" }}>
      ...
      <p className="gg-muted" style={{ marginBottom: "var(--sp-6)" }}>
        We couldn&apos;t load sale returns right now. This is usually temporary — try again in a moment.
      </p>
